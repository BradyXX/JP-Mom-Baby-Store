...
export default function CheckoutPage() {
  ...
  // Calculate Totals
  // Explicitly type as number to avoid "never" type inference in unreachable else branch
  const shippingFee: number = 0; // Fixed as per requirements
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      // Check if discount amount is > 0 based on cart content
    ...
    try {
      ...
      const orderPayload: Partial<Order> = {
        ...
        items: items.map(item => ({
          sku: 'SKU-PENDING', // Ideally item.sku, but CartItem doesn't enforce it yet
          title: item.title,
          price: item.price,
          qty: item.quantity,
          image: item.image,
      ...
      // 5. Save Summary to Storage for Success Page
